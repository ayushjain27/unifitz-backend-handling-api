...
//     // Set your HTML content for the PDF
//     const htmlContent = `
//       <html>
//         <body>
//           <h1>Sample PDF</h1>
...
app.get('/generate-pdfs', async (req, res) => {
  try {
    ...
    const jobCardId = req.query.jobCardId as string;
    const invoiceId = req.query.invoiceId as string;
    const title = req.query.title as string;

    // Fetch data first
    const [store, jobCard, invoiceCard] = await Promise.all([
      Store.findOne({ storeId }, { 'store.verificationDetails.verifyObj': 0 }),
      JobCard.findOne({ _id: new Types.ObjectId(jobCardId) }),
      CreateInvoice.findOne({ _id: new Types.ObjectId(invoiceId) })
    ]);
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=${`${title}_${jobCard?.jobCardNumber}` || 'document'}.pdf`
    );
    ...
    // Calculate next service date (6 months from creation)
    const nextServiceDate = formatDate(
      new Date(
        new Date(jobCard.createdAt).setMonth(
          new Date(jobCard.createdAt).getMonth() + 6
        )
      )
    );

    const vehicleInfo =
      (jobCard.customerDetails[0]?.storeCustomerVehicleInfo[0] as any) || {};
    const {
      brand,
      modelName: model,
      totalKmsRun: kmsDrive,
      fuelType
    } = vehicleInfo;
    ...
    function addFooter() {
      const footerY = 750; // Single Y-coordinate for all footer elements
      const lineY = footerY - 10; // Line position above the text

      // Draw the horizontal line
      doc.moveTo(40, lineY).lineTo(580, lineY).lineWidth(0.5).stroke();

      // // "Powered by ServicePlug" text

      // Calculate text vertical position accounting for font metrics
      const textBaseline = footerY - 2; // Adjusted for proper vertical alignment

      // "Powered by ServicePlug" text
      // doc.fontSize(10)
      //    .text('Powered by ServicePlug', 40, textBaseline, { align: 'left' });
      doc
        .text(
          'Powered by ServicePlug',
          40,
          footerY - 2, // Slightly adjusted for visual centering
          { width: '100%', align: 'left' }
        )
        .link(540, footerY - 2, 15, 15, 'https://google.com');
      doc.text(
        'Download',
        480,
        footerY - 2, // Slightly adjusted for visual centering
      ...
      // doc
      //   .fontSize(10)
      //   .text('Powered by ServicePlug', 40, footerY, {
      //     align: 'left'
      //   });

      // // "Download" text with link
      // doc
      //   .fontSize(10)
      //   .text('Download', 430, footerY - 2, { width: 100, align: 'right' })
      //   .link(430, footerY, 100, 15, 'https://google.com');

      // First icon with link
      doc
        .image(
      ...
      doc
        ...
        .link(560, footerY - 2, 15, 15, 'https://google.com');
    }
    ...
    doc
      .font('Helvetica-Bold')
      .fontSize(16)
      .text(store?.basicInfo?.businessName, 40, doc.y + 20, {
        width: 500,
        align: 'left'
      });
    doc
    ...
    doc
      .font('Helvetica')
      .text(jobCard?.customerDetails[0]?.name, 40, doc.y + 5, {
        width: 180,
        align: 'left'
      });
      ...
      for (const values of jobCard.lineItems) {
        ...
        if (y + rowHeight > doc.page.height - 50) {
          ...
          doc
            ...
            .stroke();
          // addFooter();
          y = 150; // Reset Y position after new page
        }
...
app.get('/generate-invoices', async (req, res) => {
  try {
    ...
    // Fetch data first
    const [store, invoiceCard] = await Promise.all([
      Store.findOne({ storeId }, { 'store.verificationDetails.verifyObj': 0 }),
      Invoice.findOne({ _id: new Types.ObjectId(invoiceId) })
    ]);
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=${`invoice_${invoiceCard?.invoiceNumber}` || 'document'}.pdf`
    );
    ...
    function addHeader() {
      ...
      doc
        .fontSize(24)
        .text('Invoice', 390, doc.y - 3, { width: 200 });
      doc
        .fontSize(12)
        .text(
          `Invoice No: ${invoiceCard?.invoiceNumber}`,
          390,
          doc.y,
          { width: 150 }
        );
      doc
        .fontSize(12)
        .text(`Invoice Date: ${invoiceDate}`, 390, doc.y, { width: 150 });
      doc
        .moveTo(35, doc.y)
        .lineTo(580, doc.y)
        .lineWidth(0.5) // Thinner line
    ...
    doc
      .font('Helvetica')
      .text(invoiceCard?.name, 40, doc.y + 5, {
        width: 180,
        align: 'left'
      });
    doc.text(invoiceCard?.address, 40, doc.y + 5, {
      width: 300,
      align: 'left'
    });
    doc.text(invoiceCard?.email, 40, doc.y + 8, {
      width: 180,
      align: 'left'
    });
    doc.text(invoiceCard?.phoneNumber, 40, doc.y + 5, {
      width: 180,
      align: 'left'
    });
    ...
    if (!isEmpty(invoiceCard?.lineItems)) {
      for (const values of invoiceCard.lineItems) {
        const { item, quantity, rate } = values;
